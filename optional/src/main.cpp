...
void mouse_scroll_callback(GLFWwindow* window, double xoffset, double yoffset) {
	...
	if (currentID != -1 || currentID != Joint_List.size()) {
		if (temp > 0) {
			Joint_List[currentID].radius += 0.0003;
		}
		else {
			Joint_List[currentID].radius -= 0.0003;
			if (Joint_List[currentID].radius <= 0.0f) {
				Joint_List[currentID].radius = 0.0f;
			}
		}
	}
}
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		for (int i = 0; i < Joint_List.size(); ++i) {
			...
			if (showSphere) {
				glBindVertexArray(sphereVAO);
				model = glm::translate(model, glm::vec3(get_parent_globalM(Joint_List[i]) * glm::vec4(Joint_List[i].local_position, 1.0f)));
				model = glm::scale(model, glm::vec3(Joint_List[i].radius));
		...
		if (isBuilt) {
			for (int i = 0; i < Joint_List.size(); ++i) {
				std::string joint_position("jointList[].position");
				joint_position.insert(10, std::to_string(i));
				...
				std::string joint_radius("jointList[].radius");
				joint_radius.insert(10, std::to_string(i));
				std::string joint_sonID("jointList[].sonID");
				joint_sonID.insert(10, std::to_string(i));
				(*model_shader).setVec3(joint_position, Joint_List[i].position);
				...
				(*model_shader).setFloat(joint_radius, Joint_List[i].radius);
				if(!Joint_List[i].child_list.empty())
					(*model_shader).setInt(joint_sonID, Joint_List[i].child_list[0]);
				else
					(*model_shader).setInt(joint_sonID, -1);
			}
		}
		else {
...
void setDefaultJoints() {
	...
	c = glm::vec3(r, g, b);
	newj = Joint(1, 0, glm::vec3(0.052980, 0.442547, 0.160829), c);
	...
	b = ((double)rand() / (RAND_MAX));
	c = glm::vec3(r, g, b);
	newj = Joint(2, 1, glm::vec3(0.073885, 0.371108, 0.159506), c);
	...
	b = ((double)rand() / (RAND_MAX));
	c = glm::vec3(r, g, b);
	newj = Joint(3, 2, glm::vec3(0.070374, 0.232418, 0.290771), c);
	...
	b = ((double)rand() / (RAND_MAX));
	c = glm::vec3(r, g, b);
	newj = Joint(4, 3, glm::vec3(0.070706, 0.152662, 0.378457), c);
	...
	b = ((double)rand() / (RAND_MAX));
	c = glm::vec3(r, g, b);
	newj = Joint(5, 4, glm::vec3(0.069522, 0.105653, 0.415685), c);
	Joint_List[4].child_list.push_back(5);
	Joint_List.push_back(newj);
}
